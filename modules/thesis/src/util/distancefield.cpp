...
static cv::Mat maskDistanceTransform(int foreground, const cv::Mat &image, bool inside) {
    ...
    const int infinity = cols + rows + 1;
...
static cv::Mat dtMeijsterRoerdinkHesselink(int foreground, const cv::Mat &image, bool inside) {
    const int cols = image.cols;
    const int rows = image.rows;
    const int infinity = cols * cols + rows * rows; // or anything larger than this
    ...
    for (int y = 0; y < rows; y++) {
        ...
        const int *vdtRow = vdt.ptr<int>(y);
        for (int x = 1; x < cols; x++) {
            int vsq = vdtRow[s[q]];
            const int vxy = vdtRow[x];
            while ((q >= 0) &&
            ...
            if (q < 0) {
                ...
                const int w = 1 + (x * x - s[q] * s[q] + vxy - vsq) / (2 * (x - s[q]));
                if (w < cols) {
                    q++;
                    s[q] = x;
                    t[q] = w;
...
cv::Mat signedDistField(const cv::Mat &img, int val) {
    const cv::Mat outside = dtMeijsterRoerdinkHesselink(val, img, false);
    const cv::Mat inside = dtMeijsterRoerdinkHesselink(val, img, true);
